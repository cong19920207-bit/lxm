...
- **鉴权**：Bearer Admin JWT；角色 **`super_admin` / `ai_trainer`**（其余 → **HTTP 403**）。
...
- **实现状态**：已实现。`activeKey='knowledge'`，顶栏标题「角色知识库」。仅 `super_admin` / `ai_trainer` 可访问，其余角色跳转 `error.html?type=403`。
