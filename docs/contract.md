...
- **响应**：**非 JSON 信封**；成功为 `StreamingResponse`（`text/event-stream`）。SSE 事件（JSON 行）包括但不限于：
  ...
  - **H5 实现说明（`frontend/pages/chat.html`）**：每次发起 **`send` / `resend`** 前递增本地 **`chatSendSession`** 并 **`AbortController`** 打断上一请求；在 **`fetch` 得到成功、且 `Content-Type` 表明为 `text/event-stream`（非 JSON 业务信封）** 之后、**进入 `consumeChatSse` 读取 SSE 正文之前**，将 **`sending=false`**，以便**未完成 SSE 消费即可再次 `send` 或叹号 `resend`**（仍受未闭环条数上限与叹号例外等既有前端校验）；`consumeChatSse` 内对响应体首段非空字节再次置 **`sending=false`** 为幂等。`consumeChatSse` 仅当传入的会话快照与 **`chatSendSession`** 一致时继续解析；收到 **`meta`** 后记录本连接 **`generation_id`** 与 **`message_count`**；若 **`delta`** 携带 **`generation_id`** 且与 **`meta`** 不一致则丢弃该条。服务端 DB / Redis 仍为权威真相，本段仅约束端上展示不串台。
