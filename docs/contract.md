...
- **fields**：`content` = `key` + 全角冒号 `：` + `value`；**`stable_key`** = key 原文（供列表展示与 keyword 匹配）；`type` 由 `dashvector_client.upsert` 合并写入；`user_id` 仅 `character_private` / `user`（Step6 自动写入，后台不可管）。
