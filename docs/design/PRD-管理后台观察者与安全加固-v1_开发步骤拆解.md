...
| F-030 | 观察者账号管理页与 API 专项验证 | [核心] | STEP-030 |
...
| STEP-002 | token_version 数据库字段与迁移 | `backend/models/admin_user.py`、`alembic/versions/`、`tests/` | 无 | 中 |
...
| 环节编号 | 功能名称 | 涉及模块 | 前置环节 | 预计复杂度 |
|---------|---------|---------|---------|----------|
| STEP-018 | observer 角色与账号管理最小扩展 | `backend/schemas/admin_auth.py`、`backend/models/admin_user.py`、`backend/routers/admin/accounts.py`、`admin/pages/accounts.html` | STEP-017 | 中 |
...
| STEP-030 | 观察者账号管理页与 API 专项验证 | `admin/pages/accounts.html`、`admin/static/js/admin-api.js`、`backend/routers/admin/accounts.py`、`tests/` | STEP-029 | 中 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 显式设置非占位的独立 `ADMIN_JWT_SECRET`，分别调用配置读取并启动应用 lifespan | 密钥可读取，应用可启动 |
| 拒绝与失败验证 | 依次测试环境变量缺失、空值、纯空白、`admin_secret_change_me`、`your_admin_jwt_secret_here` | 五种输入均明确拒绝，不回退公开默认值 |
| 边界与回归验证 | 对同一组合法/非法密钥分别经过配置读取入口和 lifespan 启动入口 | 两个入口复用同一校验函数且结论一致 |
...
**开发任务**：
1. 更新 ORM 模型，新增 `token_version INTEGER NOT NULL DEFAULT 0`。
2. 新增符合仓库 Alembic 链路的迁移，现有行回填 0。
3. 增加迁移测试，并按 `upgrade head → 字段检查 → downgrade -1 → 再次 upgrade head` 验证升级与回滚链路。

**不在本环节范围内**：
- 不为 `observer` 角色新增数据库迁移。
- 不新增测试凭证、草稿版本或发布状态表。
- 不同步或修复历史 `scripts/schema_ddl.sql` 快照；该文件不属于本 PRD 的数据库交付范围。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 在含现有管理员行的 MySQL 8 测试库执行 `alembic upgrade head`，再读取旧行并创建新行 | 新旧行 `token_version` 均为 0、非空，数据库默认值为 0 |
| 拒绝与失败验证 | 检查迁移后的列定义及全部历史管理员行 | 不存在 NULL 或缺失 `token_version` 的管理员行 |
| 边界与回归验证 | 依次执行 `upgrade head → 字段检查 → downgrade -1 → upgrade head` | 升级、回滚和再次升级成功；不修改 `scripts/schema_ddl.sql` |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 为 `token_version=0` 的有效管理员签发含版本 0 的 Admin JWT 并访问受保护接口 | 鉴权通过，JWT 中含 `token_version=0` |
| 拒绝与失败验证 | 分别使用无 `token_version`、版本非整数、版本与数据库不一致的 Admin JWT | 均返回 401，不进入业务端点 |
| 边界与回归验证 | 使用版本匹配但账号不存在、停用或锁定的 Token | 三种账号状态均返回 401；原有 `type=admin` 隔离继续有效 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 同一管理员先产生 1～4 次错密，再提交正确密码 | 登录成功，`login_fail_count` 清零且账号未锁定 |
| 拒绝与失败验证 | 对同一管理员串行提交第 5 次错误密码 | 本次请求锁定账号，失败计数保持 5，`token_version` 仅递增一次 |
| 边界与回归验证 | 已锁定账号继续提交错误或正确密码 | 对外仍为统一失败；计数保持 5，版本不重复递增 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 分别请求不存在账号、错误密码、锁定账号和停用账号登录 | 均返回 HTTP 200、业务码 `20001`、`data: null`、消息“账号或密码错误” |
| 拒绝与失败验证 | 检查四类失败响应的完整 JSON 和耗时保护调用 | 响应不含剩余次数、锁定或停用状态；不存在账号执行伪密码哈希校验 |
| 边界与回归验证 | 检查四类失败对应的服务端安全日志 | 日志可区分真实原因，但提交密码、Token、Secret 等凭据不以明文写入 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 同一账号持有两个旧 Token，使用其中一个正确完成自助改密 | 数据库版本递增一次，两个旧 Token 随后访问均为 401 |
| 拒绝与失败验证 | 分别提交旧密码错误、新旧密码相同、两次新密码不一致、强度不符 | 沿用原业务错误码，密码与 `token_version` 均不变化 |
| 边界与回归验证 | 改密后使用新密码重新登录并访问受保护接口 | 获得携带新版本的 Token，鉴权正常 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 使用有效 Admin JWT 调用 `POST /api/admin/auth/logout` | 返回成功，数据库版本递增一次，发起登出的 Token 随后为 401 |
| 拒绝与失败验证 | 同一账号预先签发多个 Token，再从任一设备登出 | 该账号其他设备旧 Token 同时为 401，其他管理员 Token 不受影响 |
| 边界与回归验证 | 登出后使用原密码重新登录 | 获得新版本 Token 并可正常访问 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | super_admin 分别重置他人密码、把他人角色改为不同合法角色 | 每个实际安全变更各递增一次 `token_version`，对应旧 Token 为 401 |
| 拒绝与失败验证 | 仅修改备注、提交与原值相同的角色、执行解锁 | 三种操作均不额外递增版本；账号管理原有权限与错误码不变 |
| 边界与回归验证 | 账号锁定后由 super_admin 解锁，再使用锁定前 Token | 解锁不恢复旧 Token；重新登录后新 Token 正常 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 合法发布角色向 `POST /api/admin/life-config/publish` 提交 `confirm_text="CONFIRM"` | 进入原发布流程并按原规则成功 |
| 拒绝与失败验证 | 分别缺失字段、传空值、纯空白或非 `CONFIRM` 文本 | 均返回业务码 `20021`，不改变草稿、生效版本、历史或 Redis |
| 边界与回归验证 | 无发布权限角色提交正确 `CONFIRM` | 仍为 403；CONFIRM 不替代角色鉴权 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 静态扫描当前 7 处 `/life-config/publish` 调用，并逐入口完成合法发布交互 | 7 处请求体均显式传递用户输入的 `confirm_text` |
| 拒绝与失败验证 | 从任一调用中移除 `confirm_text` 或改为固定伪造值的测试样例 | 静态契约测试失败，不允许遗漏入口 |
| 边界与回归验证 | 对 7 个页面分别取消确认、输入错误值和输入正确值 | 取消/错误不发布；正确值沿用原草稿与发布交互 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 输入包含大小写变体凭据键的嵌套字典、数组和可解析 JSON 字符串 | 目标凭据值递归替换为 `[REDACTED]`，结构保持可用 |
| 拒绝与失败验证 | 输入非 JSON 字符串中的已知凭据赋值、Authorization 和 Bearer 形式，并模拟单字段处理异常 | 命中值被遮蔽；异常字段失败关闭，但调用方业务不因脱敏异常失败 |
| 边界与回归验证 | 对脱敏结果重复调用，并加入 Prompt、对话、记忆、`max_tokens` 等非凭据字段 | 结果幂等，非凭据内容不变 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 通过 `log_operation` 写入 before/after/target 中含嵌套凭据的操作日志 | 入库前目标凭据均为 `[REDACTED]` |
| 拒绝与失败验证 | 直接插入一条模拟历史明文记录，再调用列表、详情和导出 | 三个读取出口均遮蔽凭据，原数据库历史行不被批量重写 |
| 边界与回归验证 | 写入和读取包含 Prompt、描述、版本、`max_tokens` 的日志 | 非凭据字段原样返回，列表分页与导出格式不回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 构造包含 API Key、Secret、Token、Password、Authorization 等内容的系统日志并请求列表 | 返回消息中的目标凭据均为 `[REDACTED]` |
| 拒绝与失败验证 | 对同一批模拟日志调用系统日志导出 | 导出文件中不出现凭据原文，列表与导出使用同一脱敏工具 |
| 边界与回归验证 | 构造普通日志、Prompt、对话说明和 `max_tokens` 内容 | 非凭据文本不误伤，既有排序、过滤和日期范围保持正常 |
...
3. 补齐操作日志与系统日志脱敏的正向、拒绝、边界与回归验证并运行相关测试。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 运行 STEP-001～013 新增/调整的 JWT、登录、CONFIRM、脱敏和账号安全 pytest | 全部通过，并记录真实命令、通过数和失败数 |
| 拒绝与失败验证 | 任一安全用例失败或未执行 | 记录失败名称与原因，STEP-014 和阶段 A 均不得标为完成或可发布 |
| 边界与回归验证 | 运行现有后台认证、账号、日志、配置及四角色权限相关测试 | 既有合法能力无回归；旧测试中与新统一响应冲突的断言已按 PRD 更新 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 在独立非生产 MySQL 8 中对同一管理员并发提交错误密码，并记录请求数与最终行状态 | 行锁使计数不丢失，达到第 5 次后账号锁定 |
| 拒绝与失败验证 | 并发量达到或超过阈值，同时读取 `is_locked`、`login_fail_count`、`token_version` | 锁定后计数为 5，版本只按锁定事件递增一次 |
| 边界与回归验证 | 锁定后继续并发错密，并另测不同管理员同时登录 | 已锁账号状态不再变化；不同账号不被同一行锁相互串行；SQLite 结果不替代本验证 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 使用 `super_admin`、`ops_admin`、`ai_trainer`、`tech_ops` 执行阶段 A 涉及的原有合法登录和业务操作 | 四角色合法能力无回归，阶段 A 新安全规则生效 |
| 拒绝与失败验证 | 对 VULN-002/003/004/005/006/008 响应统一部分/009 逐项核对测试与 MySQL 证据 | 每项均有真实命令、结果和通过/失败数量；缺证据即不得通过门禁 |
| 边界与回归验证 | 核对漏洞专档和测试报告中的未修项 | VULN-001、VULN-007、恶意锁号及其他延期风险仍明确未修或接受风险 |
...
**开发任务**：
1. 取得人工确认后同步阶段 A 已验证契约与长期规则；`.cursorrules` 只更新已生效的安全规则，角色体系仍保持现有四角色。
2. 独立部署阶段 A 前后端，保留 `token_version` 兼容字段并通知全员重新登录。
3. 记录运行稳定性确认；未确认前将阶段 B 标为阻塞。

**不在本环节范围内**：
- 不提前把 `observer` 写成已上线。
- 不在阶段 A 把 `.cursorrules` 的四角色改为五角色。
- 不把 A/B 合并为一次开发或部署。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 文档同步验证 | STEP-016 全部通过后对照真实代码、测试和部署结果更新契约、漏洞状态及 `.cursorrules` | 只写入阶段 A 已生效安全项；`.cursorrules` 仍保持四角色，不出现 observer 已上线表述 |
| 部署验证 | 独立部署阶段 A 前后端，并使用部署前旧 Token 和重新登录后的新 Token 验证 | 旧 Token 全部失效；四角色均可重新登录并执行原有合法操作 |
| 人工门禁验证 | 记录部署时间、稳定性观察结果、确认人和确认日期 | 未取得明确人工确认前，进度文档中的阶段 B 保持未开放 |
...
3. 在账号管理页加入 observer 下拉选项和列表角色标签；公共菜单与 Header 统一由 STEP-029 处理。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | super_admin 通过账号 API/页面创建 observer，再执行实际角色变更、重置、解锁和删除 | observer 合法入库并显示“观察者”，账号管理操作沿用 super_admin 权限 |
| 拒绝与失败验证 | 使用 ops_admin、ai_trainer、tech_ops、observer 直接调用全部账号管理 API | 均为 403，不返回账号数据 |
| 边界与回归验证 | 创建和编辑现有四角色账号，并检查账号页下拉选项和列表标签 | 四个旧角色与新增 observer 均可正确校验和展示；本 STEP 不修改公共菜单或 Header |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 对普通 Admin 端点分别发送 POST、PUT、PATCH、DELETE 请求 | 在进入端点业务前统一返回 403，数据库和配置状态不变化 |
| 拒绝与失败验证 | observer 分别调用 `POST /api/admin/auth/logout`、`POST /api/admin/auth/change-password`，并对相似但非精确路径发 POST | 两个精确例外进入原业务校验；相似路径仍为 403 |
| 边界与回归验证 | 使用伪造、过期、锁定、停用、无版本或版本不匹配 Token 发写请求 | 先按鉴权规则返回 401，不因 observer 方法总闸掩盖身份错误 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 不携带 Token，对 Admin 路径发送带 Origin 和预检头的 OPTIONS | CORS 中间件正常返回预检响应，不返回业务数据 |
| 拒绝与失败验证 | 不携带 Token，对同一路径发送 GET、HEAD 及普通写方法 | GET/HEAD 和写方法仍按后台鉴权返回 401 |
| 边界与回归验证 | 对具有写副作用的端点发送 OPTIONS，并检查数据库、日志和配置状态 | OPTIONS 不进入业务函数，不产生业务、账号、权限或配置副作用 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 分别调用操作日志、数据报表、系统日志三个内建导出端点 | 三个端点均返回 403，不生成或返回文件 |
| 拒绝与失败验证 | 使用各导出端点原有合法角色和合法查询参数调用 | 原有角色仍获得正确导出响应，脱敏规则继续生效 |
| 边界与回归验证 | 自动枚举当前及未来标记为导出/下载的 Admin 路由并检查观察者拒绝依赖 | 三个现有端点全部命中；新增端点缺少拒绝依赖时测试失败，规则不依赖 HTTP 方法 |
...
3. 保持用户 Open API Key 敏感显示另由 STEP-027 处理。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 调用本 STEP 确认的用户列表/详情、统计、操作日志列表与详情 GET/HEAD | 返回原业务数据；操作日志敏感内容已脱敏 |
| 拒绝与失败验证 | observer 调用同模块所有 POST、PUT、PATCH、DELETE 及三个导出中的相关端点 | 写接口和导出仍为 403，数据无变化 |
| 边界与回归验证 | 用原有四角色逐项执行其现有合法和非法权限用例 | 四角色权限矩阵不扩权、不误伤；Open API Key 状态化留给 STEP-027 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 调用人格、Prompt、Step5.5、内容安全、测试用例和只读对话流 Prompt 的读取/历史 GET | 已确认读取端点成功，响应不触发测试或生成 |
| 拒绝与失败验证 | observer 调用测试、保存草稿、发布、回滚、删除及其他写端点 | 全部为 403，配置与版本状态不变化 |
| 边界与回归验证 | 使用 super_admin、ai_trainer 及其他原有角色执行既有合法/非法用例 | 四角色权限和发布流程无误伤，VULN-001 仍明确未修 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 调用记忆、向量、知识、Agent、关系、日记、情绪及对话流只读展示端点 | 本 STEP 确认的读取端点成功，查询和分页参数有效 |
| 拒绝与失败验证 | observer 调用上述模块的保存、测试、新增、编辑、删除、导入及其他写端点 | 全部为 403，数据库、DashVector 和配置状态不变化 |
| 边界与回归验证 | 用原有四角色执行对应模块的合法读取、写入和拒绝用例 | 原权限矩阵不变；页面归属按 PRD 的 Step6、Embedding、召回/Token 定义验证 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 调用系统监控、系统日志列表和第三方配置状态读取端点 | 读取成功；系统日志内容已脱敏 |
| 拒绝与失败验证 | observer 调用系统更新、第三方测试/保存及系统日志导出 | 均为 403，不执行外部测试、不写配置、不返回文件 |
| 边界与回归验证 | tech_ops 和 super_admin 执行现有合法读取、更新、测试和导出用例 | 原有能力不受影响，其他角色不被额外扩权 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 调用 life-config、生活计划、朋友圈内容/评论、感知队列、世界观等已确认 GET/HEAD | 六类生活流数据可读取，筛选、分页和详情参数有效 |
| 拒绝与失败验证 | observer 调用生成、草稿、发布、回滚、新增、编辑、删除、重试、重置等生活流写端点 | 全部为 403，不改变 MySQL、Redis 或队列状态 |
| 边界与回归验证 | 用 ops_admin 验证既有只读范围，并用 super_admin/ai_trainer 验证既有写权限 | 原有 ops 只读与合法写角色能力不变 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 预置第三方凭据和用户 Open API Key，再以 observer 请求对应状态接口 | 仅返回“已配置”状态，不返回值、首尾字符或哈希 |
| 拒绝与失败验证 | 清空对应凭据后以 observer 请求，并尝试调用保存、更新或测试端点 | 读取仅返回“未配置”；写入和测试仍为 403 |
| 边界与回归验证 | 对响应体和序列化结果扫描原文、首尾片段、掩码片段及哈希，同时用原有合法角色回归 | observer 响应不含可识别凭据；原有角色权限按 PRD 保持 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 从 FastAPI 路由表枚举除登录外的全部 `/api/admin/*` 路由，分别执行匿名和合法角色请求 | 除匿名 CORS OPTIONS 外均有 Admin JWT 鉴权，无漏鉴权路由 |
| 拒绝与失败验证 | observer 直接请求全部写方法、账号 API 和导出端点 | 无法绕过前端写入、管理账号或导出，均按规则拒绝 |
| 边界与回归验证 | 对全部 GET/HEAD 比较请求前后的业务、权限、账号、配置状态，并单独核对有限 TTL 缓存回填 | 不产生禁止副作用；仅允许 PRD 规定的权威读取派生缓存回填 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 在 observer 会话渲染公共菜单和 Header，并对标记的按钮、文本框、选择框、开关、文件控件应用只读助手 | 菜单不含账号管理，Header 显示“观察者”；控件按类型隐藏、只读或禁用 |
| 拒绝与失败验证 | observer 使用 `adminRequest` 发 POST/PUT/PATCH/DELETE，并分别调用登出和自助改密 | 普通非 GET 在前端被阻止；两个精确例外可发送；后端 403 仍是最终边界 |
| 边界与回归验证 | 首屏后动态插入写控件，同时操作搜索、筛选、分页、Tab、详情、复制和纯 GET 刷新 | 动态写控件同样受控，所有读取交互保持可用，原有四角色不被统一只读 |
...
> 3. 提示开发者：**下一个环节是 STEP-030 观察者账号管理页与 API 专项验证**。
> 4. 若下一环节属于阶段 B，必须再次确认 STEP-017 的人工门禁；不得自动越过。

### [STEP-030] 观察者账号管理页与 API 专项验证

**目标**：专项验证现有账号页守卫与账号 API 是否满足 observer 禁入要求；现状已满足时不强制修改代码。
...
**开发任务**：
1. 验证 accounts 页面现有“非 `super_admin` 立即跳转”守卫在 observer 下先于账号数据请求执行；仅在验证失败时最小修正。
2. 验证 STEP-029 的 observer 菜单不含账号管理。
3. 自动化覆盖 GET、创建、编辑、重置、解锁、删除账号 API，确认 observer 均为 403；现有后端依赖已满足时只补测试、不改业务代码。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 直接打开 `accounts.html` 并监听首个账号请求 | 现有非 super_admin 守卫立即跳转 403 页，未发送 `/api/admin/accounts`；已满足时不改页面代码 |
| 拒绝与失败验证 | observer 直接调用账号列表、创建、编辑、重置密码、解锁和删除 API | 全部返回 403，不返回账号列表或账号详情 |
| 边界与回归验证 | super_admin 打开账号页并执行现有账号管理 API；同时核对 observer 菜单 | super_admin 页面/API 正常，observer 菜单无账号管理；只在验证失败处做最小修正 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 逐页直访 `users.html`、`user-detail.html`、`data-report.html`、`operation-logs.html`、`system-logs.html`，执行首屏加载、筛选、分页、Tab 和详情 | 五页可进入并加载已授权读取数据，日志内容已脱敏 |
| 拒绝与失败验证 | observer 检查五页静态/动态写控件和三个相关导出入口，并绕过页面直接发请求 | 写入与导出控件不可用；直接写入和导出由 API 返回 403 |
| 边界与回归验证 | 对五页运行静态契约测试，并以原有四角色执行其合法页面操作 | 动态行操作和弹窗提交均被覆盖；四角色合法能力无回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 逐页直访 `system-monitor.html`、`third-party.html`、`test-tool.html`、`dashboard.html` 并执行读取、筛选、刷新和状态查看 | 四页数据可加载，读取交互保持可用 |
| 拒绝与失败验证 | observer 检查并尝试触发第三方保存/测试、AI 测试生成、系统更新及其他写控件 | 静态和动态写控件不可用；绕过前端的请求由后端拒绝 |
| 边界与回归验证 | 对第三方已配置/未配置两种状态检查页面文本，并用原有四角色回归 | observer 只见配置状态、不见凭据；原有合法角色操作正常 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 逐页直访人格、Prompt、Step5.5 开关、安全规则及 4 个对话流 Prompt 页面，执行 Tab、历史、详情和纯读取刷新 | 八页读取正常，只读展示内容可见 |
| 拒绝与失败验证 | observer 检查并尝试触发保存、测试、发布、回滚、删除、导入及动态弹窗提交 | 所有写入口不可用；直接请求由后端拒绝 |
| 边界与回归验证 | 对八页运行静态契约测试并以原有四角色执行其合法操作 | 动态入口覆盖完整，四角色原有读取、测试和发布能力无误伤 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 逐页直访 `memory-rules.html`、`vector-token-config.html`、`knowledge.html`，执行 Tab、查询、分页和只读展示 | 三页读取与查询正常 |
| 拒绝与失败验证 | observer 检查并尝试触发保存、测试、删除、新增、编辑及动态行操作 | 所有写入口不可用；直接请求由后端拒绝 |
| 边界与回归验证 | 核对三页展示内容及原有角色操作 | `memory-rules` 为 Step6/全局记忆/DashVector，`third-party` 承担 Embedding，`vector-token-config` 不含凭据；原有角色无回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 逐页直访 `agent-rules.html`、`relationship-rules.html`、`diary-rules.html`、`diary-history.html`，执行读取、筛选、分页和详情 | 四页数据可加载，日记历史读取交互正常 |
| 拒绝与失败验证 | observer 检查并尝试触发规则保存、影响确认、关键词保存及动态提交 | 写入口不可用；绕过前端的写请求由后端拒绝 |
| 边界与回归验证 | 运行四页静态契约测试，并以原有角色执行规则管理与日记历史读取 | 动态控件覆盖完整，筛选分页可用，原有角色无回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 直访 `life-plan.html`、`worldview.html`，执行日期切换、分页、详情和只读刷新 | 两页进入和数据加载正常，读取交互保持可用 |
| 拒绝与失败验证 | observer 检查并尝试触发生成、新增、编辑、删除和配置保存 | 静态/动态写控件不可用；直接写请求由后端拒绝 |
| 边界与回归验证 | 以 ops_admin 验证既有只读范围，以 super_admin/ai_trainer 验证既有写操作 | 原有角色权限和页面交互无回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 直访 `feed-posts.html`、`feed-comments.html`、`agent-aware.html`，执行列表、筛选、分页、详情和刷新 | 三页列表和详情可用 |
| 拒绝与失败验证 | observer 检查并尝试触发内容编辑/显隐/发帖、评论编辑/软删/补发、感知重试/删除/重置 | 所有静态和动态写操作不可用；直接请求由后端拒绝 |
| 边界与回归验证 | 对动态表格行和弹窗执行静态契约/人工验证，并用原有四角色回归 | 动态入口无遗漏，四角色合法操作无回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | observer 直访 `life-feed-global.html`、`life-feed-prompts.html`、`life-feed-system.html`，执行 Tab、配置展示、状态查看和纯 GET 刷新 | 三页读取正常，展示型文本只读，选择框/开关/文件控件禁用 |
| 拒绝与失败验证 | observer 检查并尝试触发草稿、发布、测试、保存及动态生成控件 | 所有写入口不可用；直接请求由后端拒绝 |
| 边界与回归验证 | 以原有合法发布角色逐页执行发布，并静态检查 7 个调用 | 合法发布入口继续显式发送用户输入的 `CONFIRM`，原有流程无回归 |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 正向验证 | 自动枚举 Admin 路由，以 observer 对全部允许的 GET/HEAD 和两个精确 POST 例外执行矩阵测试 | 允许读取项成功；登出和自助改密进入原业务校验 |
| 拒绝与失败验证 | observer 对全部写方法、账号 API、三个导出及敏感凭据读取执行矩阵测试 | 写入、账号和导出均拒绝；凭据只返回配置状态 |
| 边界与回归验证 | 以 super_admin、ops_admin、ai_trainer、tech_ops 对同一路由矩阵回归，并测试匿名 OPTIONS | 四角色合法权限无误伤；CORS OPTIONS 仅作匿名预检，不返回业务数据 |
...
3. 全部通过后同步 observer/RBAC 契约、漏洞与 PRD 状态；此时才把 `.cursorrules` 的后台角色体系从四角色更新为包含 observer 的五角色，并记录命令/数量/失败原因后独立部署。

**不在本环节范围内**：
- 不把防截图、复制、分页聚合描述成能力。
- 不在 observer 尚未实现和验证前提前修改 `.cursorrules` 的角色体系。
- 任何失败时不得标为可发布或已完成。
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 页面验收验证 | 对 35 页记录角色、直访、首屏加载、读取交互、静态/动态写控件和合法操作；登录页与错误页单独核对 | 35 页均有结论；observer 账号页禁入，其他授权页面只读，原有四角色无误伤 |
| 文档同步验证 | 页面与后端矩阵全部通过后，对照真实代码和测试更新契约、漏洞、PRD、进度及 `.cursorrules` | 文档只描述已验证事实；`.cursorrules` 此时才从四角色更新为包含 observer 的五角色 |
| 部署与人工确认 | 独立部署阶段 B，记录真实命令、通过/失败数量、失败原因、人工验收结果、确认人和日期 | 任一失败不得标为完成；全部通过并确认稳定后才关闭本 PRD |
