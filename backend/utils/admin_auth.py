...
_OBSERVER_BLOCKED_METHODS = frozenset({"POST", "PUT", "PATCH", "DELETE"})
_OBSERVER_SELF_SERVICE_EXCEPTIONS = frozenset({
...
async def get_current_admin(
    ...
    完成数据库实时状态与会话版本校验后，再执行 observer 方法级写总闸。
    """
...
async def deny_observer_export(
    admin_user: AdminUser = Depends(get_current_admin),
) -> AdminUser:
    """导出/下载端点专用边界；不依赖请求方法拒绝 observer。"""
...
async def log_operation(
    ...
    def redact_audit_value(value):
        ...
        try:
            ...
            logger.exception("操作日志字段脱敏失败，已按失败关闭处理")
            return REDACTED
    ...
    log_entry = AdminOperationLog(
        ...
        module=module,
        action=action,
        target_description=redact_audit_value(target_description),
        before_value=redact_audit_value(before_value),
        after_value=redact_audit_value(after_value),
